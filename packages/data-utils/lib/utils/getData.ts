...
export default async function getData<T>(
  ...
  if (!bearerRequest) {
    bearerRequest = new HttpRequest({ noCache: true });
  }

  // get from api
  let apiData: T | undefined = undefined;
  if (options.postData) {
    apiData = await bearerRequest.post<T>(path, options.postData, key);
  } else {
    apiData = await bearerRequest.get<T>(path);
  }
