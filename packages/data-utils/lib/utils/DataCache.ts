...
export default class DataCache {
  ...
  async listEntries<T extends object>(property: string): Promise<T[]> {
    if (!this.dbPromise) {
      console.error("cache list entries - no promise");
      return [];
    }
    try {
      const db = await this.dbPromise;
...
/** returns a new object with some reasonable cache options */
export const getDefaultCacheOptions = (): DataCacheOptions => {
  return {
    storeName: "cache",
    dbVersion: 1,
