...
import {
  ...
  specForMvDetail,
  specForMvTrend,
} from "@/specs/materializedViewsSpecs";

const mvColumns: SortableTableColumn<MvDetailData>[] = [
  {
...
export default function MaterializedViews() {
  ...
  const detailQuery = useMemo(() => {
    return specForMvDetail(filters);
  }, [filters]);
  const {
    data: detailData,
  ...
  const detailObjs = useMemo(() => {
    const objs = parseQueryResponse<MvTrendData>(detailData, detailQuery.columns);
    return objs;
  }, [detailData, detailQuery]);

  const trendQuery = useMemo(() => {
    return specForMvTrend(filters);
  }, [filters]);
  const {
    data: trendData,
  ...
  const trendObjs = useMemo(() => {
    const objs = parseQueryResponse<MvTrendData>(trendData, trendQuery.columns);
    const agg = aggregateByDate<MvTrendData>(objs, "logdate", periodType, trendQuery.columns);
    return agg;
        ...
        <table className={basicTable}>
          ...
          <tbody>
            {data?.sort(alphaSorter("credits_used", true)).map((row, i) => {
              const typed = row as MvTrendData;
