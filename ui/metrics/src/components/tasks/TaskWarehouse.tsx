...
export default function TaskWarehouse({ filters, db, schema }: TaskWarehouseProps) {
  const [periodType, setPeriodType] = useState<PeriodType>(PeriodType.week);
  const spec = useMemo(() => {
    return specForTaskWarehouse({ selectedValues: filters, db, schema });
  }, [filters, db, schema]);
  ...
  const objs = useMemo(() => {
    const objs = parseQueryResponse<TaskWarehouseData>(data, spec?.columns);
    const agg = aggregateByDate<TaskWarehouseData>(objs, "logdate", periodType, spec?.columns);
    return agg;
  ...
  return (
    <Box className="p-2">
      <div className="text-lg">Task Warehouse Trend</div>
        ...
        <table className={basicTable}>
          ...
          <tbody>
            {data?.sort(alphaSorter("query_credits_used", true)).map((row, i) => {
              const typed = row as TaskWarehouseData;
